...
foreach ($tickets as $ticket) {
    ...
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->MultiCell(0, 5, iconv("UTF-8", "ISO-8859-1", strtoupper("Boleta N°: " . $respuesta["serie_comprobante"] . '-' . $respuesta["num_comprobante"])), 0, 'C', false);
    $pdf->SetFont('Arial', '', 9);
...
if (isset($_GET['accion']) && $_GET['accion'] === 'descargar') {
    $pdf->Output('D', 'boleta_compra_' . $numero_comprobante . '.pdf');
} else {
    $pdf->Output();
}
?>