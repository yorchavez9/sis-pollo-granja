...
<body>
    ...
    <div class="factura-info">Factura N° '.$serieNumero.'-'.$num_comprobante.'</div>
    <div class="factura-info">Fecha: '.date("d/m/Y", strtotime($fechaEgreso)).'</div>
    <div class="factura-info">Hora: '.$horaEgreso.'</div>
...
if (isset($_GET['accion']) && $_GET['accion'] === 'descargar') {
    $dompdf->stream("factura_".$serieNumero."-".$num_comprobante.".pdf", array("Attachment" => true));
} else {
    $dompdf->stream("factura_".$serieNumero."-".$num_comprobante.".pdf", array("Attachment" => false));
}