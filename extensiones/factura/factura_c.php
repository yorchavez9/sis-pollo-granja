...
$horaVenta = $respuesta["hora_cotizacion"];
$numeroCotizacion = $respuesta["id_cotizacion"];
$respuesta_dv = ControladorCotizacion::ctrMostrarDetalleCotizacion($item, $valor);
$fechaVenta = $respuesta["fecha_cotizacion"];
...
class PDF extends FPDF
{
    ...
    function __construct($numeroCotizacion, $fechaVenta, $horaVenta, $nombreEmpresa, $ruc, $telefono, $correo, $direccion, $logo, $mensaje)
    {
    ...
    function Header()
    {
        ...
        $this->SetX(10);
        $this->Cell(0, 7, 'FACTURA COTIZACION', 0, 1, 'L');
        $this->SetFont('Helvetica', 'B', 12);
        $this->Cell(0, 7, utf8_decode('Cotizacion N° ' . $this->numeroCotizacion), 0, 1, 'L');  // Accede a la propiedad
...
// Crear PDF
$pdf = new PDF($numeroCotizacion, $fechaVenta, $horaVenta, $nombreEmpresa, $ruc, $telefono, $correo, $direccion, $logo, $mensaje);
$pdf->AliasNbPages();
$pdf->AddPage();
...
foreach ($respuesta_dv as $index => $producto) {
    ...
    $pdf->Cell(20, 10, 'USD ' . number_format($producto['precio_venta'], 2), $borde, 0, 'C');
